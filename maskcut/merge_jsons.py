...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--save-path",
        type=str,
        default="imagenet_train_fixsize480_tau0.15_N3.json",
    ...
    base_name = "imagenet_train_fixsize{}_tau{}_N{}".format(
        args.fixed_size, args.tau, args.N
    )
    ...
    for tobe_merged_ann_dict in tobe_merged_ann_dicts[1:]:
        ...
        for i, annotation_info in enumerate(tobe_merged_ann_dict["annotations"]):
            annotation_info["image_id"] = file_name_and_id[
                str(annotation_info["image_id"])
            ]
    ...
    print(
        "Done: {} images; {} anns.".format(
            len(base_ann_dict["images"]), len(base_ann_dict["annotations"])
        )
    )
