...
sys.path.append("../")
import dino  # model    # noqa
from third_party.TokenCut.unsupervised_saliency_detection import utils, metric  # noqa
from third_party.TokenCut.unsupervised_saliency_detection.object_discovery import (  # noqa
    detect_box,  # noqa
)

# Modified by XuDong Wang from third_party/TokenCut
# bilateral_solver codes are modfied based on
# https://github.com/poolio/bilateral_solver/blob/master/notebooks/bilateral_solver.ipynb
# from third_party.TokenCut.unsupervised_saliency_detection.bilateral_solver
# import BilateralSolver, BilateralGrid
# crf codes are are modfied based on
# https://github.com/lucasb-eyer/pydensecrf/blob/master/pydensecrf/tests/test_dcrf.py
...
def maskcut_forward(feats, dims, scales, init_image_size, tau=0, N=3):
    ...
    bipartitions = []
    eigvecs = []
    current_mask, ps = None, None

    for i in range(N):
        if i == 0:
            painting = torch.from_numpy(np.zeros(dims)).cuda()
        else:
            feats, painting = get_masked_affinity_matrix(
        ...
        bipartition = F.interpolate(
            pseudo_mask.unsqueeze(0).unsqueeze(0), size=init_image_size, mode="nearest"
        ).squeeze()
...
def maskcut(img_path, backbone, patch_size, tau, N=1, fixed_size=480):
    ...
    I_new = Img.resize((int(fixed_size), int(fixed_size)), PIL.Image.LANCZOS)
...
def get_args_parser():
    ...
    parser.add_argument(
        "--patch-size", type=int, default=16, choices=[16, 8], help="patch size"
    )
    parser.add_argument(
        "--nb-vis", type=int, default=20, choices=[1, 200], help="nb of visualization"
    )
    parser.add_argument(
        "--img-path", type=str, default=None, help="single image visualization"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="the number of folders each job processes",
    )
    parser.add_argument(
        "--job-index",
        type=int,
        default=0,
        help="the index of the job (for imagenet: in the range of 0 to 1000/args.num_folder_per_job-1)",
    )
    parser.add_argument(
        "--fixed_size",
        type=int,
    ...
    parser.add_argument(
        "--pretrain_path", type=str, default=None, help="path to pretrained model"
    )
    parser.add_argument(
        "--N", type=int, default=3, help="the maximum number of pseudo-masks per image"
    )
    return parser
...
def main(args):
    ...
    backbone = dino.ViTFeat(
        url, feat_dim, args.vit_arch, args.vit_feat, args.patch_size
    )
    ...
    for img_folder in img_folders[start_idx:end_idx]:
        ...
        for img_name in tqdm(img_list):
            ...
            try:
                ...
                print(f"Error {e}: Skipping {img_name}")
                continue
    ...
    if len(img_folders) == args.num_folder_per_job and args.job_index == 0:
        json_name = "{}/imagenet_train_fixsize{}_tau{}_N{}.json".format(
            args.out_dir, args.fixed_size, args.tau, args.N
        )
    else:
        json_name = "{}/imagenet_train_fixsize{}_tau{}_N{}_{}_{}.json".format(
            args.out_dir, args.fixed_size, args.tau, args.N, start_idx, end_idx
        )
